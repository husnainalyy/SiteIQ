...
const createNewReport = async (req, res) => {
  ...
  try {
    ...
    if (!report) {
      ...
      const existingPhrase = report.phraseResults.find(
        (p) => p.phrase === phrase
      );
    ...
    // Step 6: Update User's seoReport in the correct website entry
    await User.updateOne(
      ...
      {
        $set: {
          "websites.$.seoReport": report.toObject(),
        },
      }
    );
    ...
    try {
      ...
      if (response.data.ready) {
        ...
        if (!report) {
          return res
            .status(404)
            .json({ error: "Report with this jid not found" });
        }
        ...
        if (!phraseResult) {
          return res
            .status(404)
            .json({ error: "Phrase result not found for jid" });
        }
      ...
      if (attempts >= maxAttempts) {
        return res
          .status(202)
          .json({
            message: "Job not ready after waiting, try again later",
            jid,
          });
      }
...
const scorePhrase = async (req, res) => {
  ...
  if (!jid || !phrase || !domain) {
    return res
      .status(400)
      .json({ error: "jid, phrase, and domain are required." });
  }
  ...
  const phraseEntry = report.phraseResults.find(
    (p) => p.jid === jid && p.phrase === phrase
  );
  if (!phraseEntry) {
